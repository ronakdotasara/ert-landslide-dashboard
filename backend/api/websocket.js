...
async function checkAnomalies(r) {
  ...
  if (rho < ALERT_CRITICAL) {
    alert = { type: 'ANOMALY', severity: 'CRITICAL', kind: 'LOW_RESISTIVITY', rho, elec_a: r.elec_a, elec_b: r.elec_b, message: `CRITICAL: ρ=${rho.toFixed(1)} Ω·m`, ts: Date.now() };
  } else if (rho < ALERT_LOW) {
    alert = { type: 'ANOMALY', severity: 'HIGH', kind: 'LOW_RESISTIVITY', rho, elec_a: r.elec_a, elec_b: r.elec_b, message: `WARNING: ρ=${rho.toFixed(1)} Ω·m`, ts: Date.now() };
  } else if (rho > ALERT_HIGH) {
    alert = { type: 'ANOMALY', severity: 'LOW', kind: 'HIGH_RESISTIVITY', rho, elec_a: r.elec_a, elec_b: r.elec_b, message: `INFO: ρ=${rho.toFixed(1)} Ω·m`, ts: Date.now() };
  }
  if (alert) {
    broadcast(alert);
...
async function checkVoltageAlert(data) {
  ...
  if (v < ZMPT_MIN || v > ZMPT_MAX) {
    const alert = { type: 'VOLTAGE_ALERT', severity: 'MEDIUM', vrms: v, message: `AC voltage out of range: ${v.toFixed(1)} V`, ts: Date.now() };
    broadcast(alert);
    try { await saveAlert(alert); } catch (_) {}
  }
}
